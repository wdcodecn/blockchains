...
class BakerySwapUtils(object):
    ...
    def sort_tokens(token_a, token_b):
        ...
        assert token_0 != BakerySwapUtils.ZERO_ADDRESS
        return token_0, token_1

    @staticmethod
    def pair_for(factory, token_a, token_b):
        prefix = Web3.toHex(hexstr="ff")
        encoded_tokens = Web3.solidityKeccak(["address", "address"], BakerySwapUtils.sort_tokens(token_a, token_b))
        suffix = Web3.toHex(hexstr="e2e87433120e32c4738a7d8f3271f3d872cbe16241d67537139158d90bac61d3")
...
class BakerySwapV2Client(BakerySwapObject):

    ADDRESS = "0x01bF7C66c6BD861915CdaaE475042d3c4BaE16A7"

    ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IBakerySwapFactory.json")))["abi"]

    ROUTER_ADDRESS = "0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F"
    ROUTER_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IBakerySwapRouter.json")))["abi"]

    MAX_APPROVAL_HEX = "0x" + "f" * 64
    MAX_APPROVAL_INT = int(MAX_APPROVAL_HEX, 16)
    ERC20_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IBakerySwapBEP20.json")))["abi"]

    PAIR_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IBakerySwapPair.json")))["abi"]

    def __init__(self, address, private_key, provider=None):
        super().__init__(address, private_key, provider)
        self.contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(BakerySwapV2Client.ADDRESS), abi=BakerySwapV2Client.ABI)
        self.router = self.conn.eth.contract(
            address=Web3.toChecksumAddress(BakerySwapV2Client.ROUTER_ADDRESS), abi=BakerySwapV2Client.ROUTER_ABI)

    # Utilities
    # -----------------------------------------------------------
    def _is_approved(self, token, amount=MAX_APPROVAL_INT):
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=BakerySwapV2Client.PAIR_ABI)
    ...
    def approve(self, token, max_approval=MAX_APPROVAL_INT):
        ...
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=BakerySwapV2Client.ERC20_ABI)
    ...
    def get_factory(self, query_chain=False):
        ...
        return BakerySwapV2Client.ADDRESS
    ...
    def get_token_0(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=BakerySwapV2Client.PAIR_ABI)
    ...
    def get_reserves(self, token_a, token_b):
        ...
        """
        (token0, token1) = BakerySwapUtils.sort_tokens(token_a, token_b)
        pair_contract = self.conn.eth.contract(
                address=self.get_pair(token_a, token_b),
                abi=BakerySwapV2Client.PAIR_ABI
            )
    ...
    def get_amounts_out(self, amount_in, path):
        ...
        for p0, p1 in zip(path, path[1:]):
            ...
            current_amount = BakerySwapUtils.get_amount_out(
    ...
    def get_amounts_in(self, amount_out, path):
        ...
        for p0, p1 in reversed(list(zip(path, path[1:]))):
            r = self.get_reserves(p0, p1)
            current_amount = BakerySwapUtils.get_amount_in(
                current_amount, r[0], r[1]
            )
