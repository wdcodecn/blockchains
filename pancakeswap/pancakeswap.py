...
class PancakeSwapUtils(object):
    ...
    def sort_tokens(token_a, token_b):
        ...
        assert token_0 != PancakeSwapUtils.ZERO_ADDRESS
        return token_0, token_1

    @staticmethod
    def pair_for(factory, token_a, token_b):
        prefix = Web3.toHex(hexstr="ff")
        encoded_tokens = Web3.solidityKeccak(["address", "address"], PancakeSwapUtils.sort_tokens(token_a, token_b))
    ...
    def get_amount_out(amount_in, reserve_in, reserve_out):
        ...
        amount_in_with_fee = amount_in*998
...
class PancakeSwapObject(object):
...
class PancakeSwapV2Client(PancakeSwapObject):

    # ADDRESS = "0xBCfCcbde45cE874adCB698cC183deBcF17952812"
    ADDRESS = "0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73"

    ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IPancakeFactory.json")))["abi"]

    # ROUTER_ADDRESS = "0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F"
    ROUTER_ADDRESS = "0x10ED43C718714eb63d5aA57B78B54704E256024E"
    ROUTER_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IPancakeRouter02.json")))["abi"]

    MAX_APPROVAL_HEX = "0x" + "f" * 64
    MAX_APPROVAL_INT = int(MAX_APPROVAL_HEX, 16)
    ERC20_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IPancakeERC20.json")))["abi"]

    PAIR_ABI = json.load(open(os.path.abspath(f"{os.path.dirname(os.path.abspath(__file__))}/assets/" + "IPancakePair.json")))["abi"]

    def __init__(self, address, private_key, provider=None):
        super().__init__(address, private_key, provider)
        self.contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(PancakeSwapV2Client.ADDRESS), abi=PancakeSwapV2Client.ABI)
        self.router = self.conn.eth.contract(
            address=Web3.toChecksumAddress(PancakeSwapV2Client.ROUTER_ADDRESS), abi=PancakeSwapV2Client.ROUTER_ABI)

    # Utilities
    # -----------------------------------------------------------
    def _is_approved(self, token, amount=MAX_APPROVAL_INT):
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=PancakeSwapV2Client.PAIR_ABI)
    ...
    def approve(self, token, max_approval=MAX_APPROVAL_INT):
        ...
        erc20_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(token), abi=PancakeSwapV2Client.ERC20_ABI)
    ...
    def get_factory(self, query_chain=False):
        ...
        return PancakeSwapV2Client.ADDRESS
    ...
    def get_token_0(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.toChecksumAddress(pair), abi=PancakeSwapV2Client.PAIR_ABI)
    ...
    def get_reserves(self, token_a, token_b):
        ...
        """
        (token0, token1) = PancakeSwapUtils.sort_tokens(token_a, token_b)
        pair_contract = self.conn.eth.contract(
            # address=Web3.toChecksumAddress(
            #     PancakeSwapUtils.pair_for(self.get_factory(), token_a, token_b)),
                address= self.get_pair(token_a,token_b),
                abi=PancakeSwapV2Client.PAIR_ABI
            )
    ...
    def get_amounts_out(self, amount_in, path):
        ...
        for p0, p1 in zip(path, path[1:]):
            ...
            current_amount = PancakeSwapUtils.get_amount_out(
    ...
    def get_amounts_in(self, amount_out, path):
        ...
        for p0, p1 in reversed(list(zip(path, path[1:]))):
            r = self.get_reserves(p0, p1)
            current_amount = PancakeSwapUtils.get_amount_in(
                current_amount, r[0], r[1]
            )
